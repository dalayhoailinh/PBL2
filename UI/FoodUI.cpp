...
void FoodUI::printFoodRow(const Food &f) {
    cout << left << setw(6) << f.id
         ...
         << setw(12) << fixed << setprecision(2) << f.price
         << endl;
}
...
void FoodUI::listFoods() {
    try {
        ...
        cout << "\n=== DANH SACH MON AN ===\n";
        if (rows.empty()) {
            cout << "Khong co mon nao trong he thong.\n";
            return;
        }
        printFoodHeader();
        for (size_t i = 0; i < rows.size(); ++i)
            printFoodRow(rows[i]);
        cout << string(70, '=') << endl;
        cout << "Tong cong: " << rows.size() << " mon\n";
    } catch (const exception &e) {
        cout << "Loi khi lay danh sach mon: " << e.what() << endl;
    }
}
void FoodUI::addFood() {
    Food f;
